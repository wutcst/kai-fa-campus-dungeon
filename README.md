...
> 在迷雾中探索随机楼层，收集答辩材料，和敌人回合制战斗，利用药水与装备成长，最终击败 Defense Committee 完成软件工程实践答辩。
...
Campus Dungeon 最初来自一个 Zuul 风格的命令行文字冒险原型。原型验证了房间、道具、背包、NPC、任务门禁、评分和结局等基础机制；随后项目升级为 BYOG-inspired 的图形化随机地牢，并进一步演进为 **Campus Dungeon: Roguelike Defense**。

当前版本已经完成轻量 Roguelike RPG 主线：Vim 键位、5 层地牢、三态迷雾、玩家属性、回合制敌人、药水装备、陷阱事件、像素风渲染、JSON 存档和最终答辩 Boss 均已落地。

## 游戏亮点

- **随机校园地牢**：输入 seed 后生成 `80 x 40` 的房间与走廊地图，同一 seed 可复现。
- **图形化探索**：使用 Java Swing 自研瓦片渲染，绘制墙体、地板、玩家方向、道具、NPC、敌人、陷阱、楼梯、Boss 和 HUD。
- **三态迷雾系统**：未探索区域全黑，已探索区域暗色显示，当前视野正常显示。
- **RPG 战斗成长**：玩家拥有 `HP / ATK / DEF / Level / EXP`，敌人会在玩家有效行动后追击或攻击。
- **多层地牢推进**：前 4 层寻找楼梯继续深入，第 5 层进入最终答辩 Boss 战。
- **药水与装备**：小药水、大药水、咖啡、三档武器和三档防具提供探索与战斗决策。
- **陷阱事件**：扣血、传送、虚弱陷阱让最短路径不再总是最优路线。
- **NPC 任务链**：`librarian`、`assistant`、`teacher` 会根据背包与任务状态给出不同反馈。
- **课程谜题机制**：通过 Maven 配置文件 `pom.xml` 谜题获得关键材料。
- **JSON 存档**：保存完整 `GameState`，包括玩家属性、楼层、地图、背包、装备、道具、NPC、敌人、陷阱、Boss、任务与探索状态。
- **输入回放测试**：保留 BYOG 风格 `playWithInputString`，核心玩法无需打开 GUI 即可自动化验证。
- **答辩通关闭环**：收集 `report`、`laptop`、`slides`、`pass` 后，在第 5 层击败 `Defense Committee` 并进入 defense hall 完成答辩。

## Roguelike Defense 玩法
...
- WASD 动作键位：`w/a/s/d` 移动，`j` 或 Space 发射 `Keyboard Pistol` 直线子弹，`o` 读取存档。
- 玩家属性：`HP / MaxHP / ATK / DEF / Level / EXP`。
- 敌人：`Bug`、`Deadline` 和最终 `Defense Committee`。
...
当前版本具备可运行的 Swing 窗口和完整 Roguelike Defense 主线。后续可在这里补充游戏窗口截图或演示 GIF，用于项目主页、课程报告和答辩视频。

```text
┌──────────────────────── Campus Dungeon ────────────────────────┐
│  黑暗迷雾覆盖未知区域，玩家从入口出发，逐步点亮校园地牢。       │
│  道具、NPC、敌人、陷阱、楼梯、Boss 会用不同颜色和符号呈现。     │
│  底部 HUD 显示 seed、depth、HP、EXP、ATK/DEF、装备和事件。      │
└────────────────────────────────────────────────────────────────┘
```
...
|---|---|
| `W` / `A` / `S` / `D` | 上、左、下、右移动 |
| `J` / Space | 发射 `Keyboard Pistol` 直线子弹，子弹按固定 tick 前进、撞墙消失、命中敌人造成伤害 |
| `K` | 技能预留，当前提示暂未解锁 |
...
o                读取存档
w/a/s/d          移动
j 或 Space       发射 Keyboard Pistol 直线子弹
k                技能预留
e                交互
i                查看背包/状态；可用时使用药水或咖啡
:q               保存并退出
!answer(pom.xml) 回答 Maven 课程谜题
!tick(n)         推进 n 个固定 tick，用于回放测试动作循环
```
...
你的目标不是简单地跑图捡物品，而是在有限生命、敌人追击和陷阱压力下完成校园答辩准备，并在最终楼层通过 Boss 战：
...
- 玩家移动、攻击、交互、使用补给等有效行动后，敌人会推进一回合。
- 药水可以恢复 HP，咖啡能临时提升下一次攻击，武器和防具会提升 ATK/DEF。
- 陷阱会造成扣血、传送或虚弱效果，触发状态会进入存档。
- 普通楼层需要找到楼梯进入下一层。
- `defense hall` 需要 `report + laptop + slides + pass`，并且必须先击败 `Defense Committee` 才能通关。

HP 归零后进入 `GAME_OVER`，读取死亡存档不会自动复活。
...
GUI KeyListener / Replay Input
   ...
   World    Entity    Quest    IO
     |        |         |       |
 Renderer  Items/NPC/Enemy/Trap  Progress JSON Save
```
...
| `cn.edu.whut.sept.dungeon.entity` | 道具、背包、NPC、敌人、陷阱 |
...
- JSON 存档恢复玩家属性、装备、楼层、地图、背包、道具、NPC、敌人、陷阱、Boss、任务和探索状态。
- 道具拾取与背包展示。
- NPC 对话、任务推进和 Maven 谜题。
- WASD movement、Keyboard Pistol projectile、固定 tick 回放推进、回合制敌人行动、近战伤害、经验获取和 `GAME_OVER`。
...
存档格式当前为 version `2`，会保存 Roguelike 动态状态。`save/` 已加入 `.gitignore`，本地游玩进度不会进入 Git。
...
当前版本已经升级为 `Campus Dungeon: Roguelike Defense`：
...
| #41 | #42 | Vim movement 与读取存档协议调整 |
| #43 | #44 | 玩家 RPG 战斗属性与游戏状态 |
| #45 | #46 | 敌人与近战战斗 |
| #47 | #48 | 回合制敌人行动 |
| #49 | #50 | 药水、咖啡、武器和防具 |
| #51 | #52 | 多层地牢、楼梯与深度推进 |
| #53 | #54 | 陷阱与地牢事件 |
| #55 | #56 | 最终 Defense Committee Boss |
| #57 | #58 | 像素风地牢渲染与 HUD |
| #59 | #60 | RPG 地牢状态完整存档 |
